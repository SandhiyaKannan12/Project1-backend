...
function splitContentToSentences(content, callback) {
  ...
  callback(_sbd.default.sentences(content, {
...
function getSortedSentences(paragraph, sentences_dict, n, callback) {
  splitContentToSentences(paragraph, function (sentences) {
    ...
    var sentence_scores = [],
      strip_s;
    _lodash.default.each(sentences, function (s, i) {
    ...
    sentence_scores = _lodash.default.sortBy(sentence_scores, function (sentence_score) {
    ...
    sentence_scores = _lodash.default.sortBy(sentence_scores, function (sentence) {
      return sentence.order;
    });
    callback(_lodash.default.map(sentence_scores, 'sentence')); // callback with sorted_sentences. _.map is former .pluck
  });
}
function getSentencesRanks(content, callback, sentences_dict) {
  ...
  splitContentToSentences(content, function (sentences) {
    var n = sentences.length,
      zeroNRange = _lodash.default.range(0, n),
      nRange = _lodash.default.range(n);

    // This is ugly, I know.
    var values = [],
      _val = [];
    _lodash.default.each(nRange, function (x) {
      _val = [];
      _lodash.default.each(nRange, function (y) {
        _val.push(0);
      });
      values.push(_val);
    });
    // Assign each score to each sentence
    _lodash.default.each(zeroNRange, function (i) {
      _lodash.default.each(zeroNRange, function (j) {
    ...
    var score = 0;
    _lodash.default.some(zeroNRange, function (i) {
      score = 0;
      _lodash.default.some(zeroNRange, function (j) {
...
exports.summarize = function (title, content, callback, sentences_dict) {
  ...
  getSentencesRanks(content, function (dict) {
    splitContentToParagraphs(content, function (paragraphs) {
      ...
      _lodash.default.each(paragraphs, function (p) {
